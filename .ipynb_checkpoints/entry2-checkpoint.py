...
def prepare(config_path):
    ...
    parser.add_argument('--task_run', default='targetel')
    ...
    parser.add_argument('--logdir', default='/root/tf-logs/two')
...
if __name__ == '__main__':
    config_path = '/root/MAREC/coding_beifen/entropy研究/config50_0.json'
...
#=====================这里是批量批量常规的训练========================
    parent_dir = '/root/tf-logs/entropy研究/MAPPO'
...
#book还有5-2,
    for ratio in [[0.2, 0.8]]:
        ...
        for seed in [10,500,900,1000,100,2020]:
            for lambda_en1 in [0,0.000000005,0.000000008,0.00000002,0.0000002,0.000001,0.0001,0.001,0.01,1,10]:
                random.seed(seed)
                np.random.seed(seed)
                ...
                Run(config,writer1).main()
                writer1.close()
#=====================================================

#===================这里是测试调参用的======================
# if __name__ == '__main__':
#     config_path = '/root/MAREC/coding_beifen/mappo宝藏钥匙别动，2_8最低能到1.27/备份一/画weight的mappo/config.json'
#     args, config = prepare(config_path)
#     print(f"cudnn enabled: {torch.backends.cudnn.enabled}")
#     print(f"number of used gpu: {torch.cuda.device_count()}")
#     print(f"cuda is available: {torch.cuda.is_available()}")
...
#     for seed in [100,10,900,2020,1000]:
#         random.seed(seed)
#         np.random.seed(seed)
#         torch.manual_seed(seed)
#         torch.cuda.manual_seed(seed)
