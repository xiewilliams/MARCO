...
def prepare(config_path):
    ...
    parser.add_argument('--ratio', nargs="+", default=[0.5, 0.5])
    ...
    parser.add_argument('--logdir', default='/root/autodl-tmp/entropy_experiment')
...
#===================这里是测试调参用的======================
# if __name__ == '__main__':
#     config_path = '/root/MAREC/coding_beifen/mappo宝藏钥匙别动，2_8最低能到1.27/备份一/画weight的mappo/config50_0.json'
#     args, config = prepare(config_path)
#     print(f"cudnn enabled: {torch.backends.cudnn.enabled}")
#     print(f"number of used gpu: {torch.cuda.device_count()}")
#     print(f"cuda is available: {torch.cuda.is_available()}")
...
#     for seed in [10,900,2020,100,1000]:
#         random.seed(seed)
#         np.random.seed(seed)
#         torch.manual_seed(seed)
#         torch.cuda.manual_seed(seed)
