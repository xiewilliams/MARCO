...
class Run():
    ...
    def read_log_data(self, path, batchsize, history=False):
        if not history:
            ...
            y_col = ['y']
            data.columns = cols
            pos_seq = keras.preprocessing.sequence.pad_sequences(data.pos_seq.map(self.seq_extractor), maxlen=250,
                                                                 padding='post')
    ...
    def CDRWithRL(self, model, data_meta, data_test, criterion, optimizer_meta,device,writer,weight_dir,performance):
        ...
        agent = MAPPO(env.state_dim, env.state_dim, env.critic_inputs,env.state_dim//2,env.action_space_n,self.rl_lr,0.95,0.95,0.2,self.lambda_en1,self.device)
        data_meta_records = self.record_num(self.meta_path)
