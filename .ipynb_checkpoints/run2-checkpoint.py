...
class Run():
    ...
    def CDRWithRL(self, model, data_meta, data_test, criterion, optimizer_meta,device,writer):
        ...
        agent = MAPPO(env.state_dim, env.state_dim, env.critic_inputs,env.state_dim//2,env.action_space_n,self.rl_lr,0.98,0.98,0.2,self.lambda_en1,self.device)
        data_meta_records = self.record_num(self.meta_path)
        ...
        for i in range(self.epoch):
            ...
            mae, rmse,_= infer.eval_mae()#看来这个要根据rmise的结果再输出,这下面的两步要根据每一次的情况都要改一下，要改12种情况
            # if rmse<1.5:
            #     df_weight['rmse']=rmse
            #     df_weight['mae']=mae
            #     df_weight.to_csv(r'/root/MAREC/coding_beifen/mappo宝藏钥匙别动，2_8最低能到1.27/备份一/画weight的mappo/rate8/weight_el'+str(rmse)+'.csv')
    ...
    def main(self):
        ...
        if self.algo == 'cmf':
            ...
            self.TgtOnly(model, data_tgt, data_test, criterion, optimizer_tgt)
            self.CDRWithRL(model, data_meta, data_test, criterion, optimizer_meta,self.device,self.writer)
