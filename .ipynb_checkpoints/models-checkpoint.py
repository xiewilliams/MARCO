...
class TargetAttention(torch.nn.Module):
    ...
    def forward_avg(self, key_value):
        srs_atten = torch.ones(key_value.shape[0], self.num_heads)# [batch, num_heads]
...
#             print(f"Loading pre-train user embedding for domain {i}")
#             if i != target_domain:
#                 model_dict= torch.load(single_dirs[i],map_location='cuda')
#                 sizes = model_dict['uid_embedding.weight'].size()
#                 sizes_id = model_dict['iid_embedding.weight'].size()
#                 user_embedding_list.append(torch.nn.Embedding(sizes[0],sizes[1],
#                                                                 _weight=model_dict['uid_embedding.weight']))
...
#         self.user_embedding_list = nn.ModuleList(user_embedding_list)
#         self.item_embedding_list = nn.ModuleList(item_embedding_list)
#         model_dict = torch.load(single_dirs[target_domain],map_location='cuda')
#         sizes_ti = model_dict['iid_embedding.weight'].size()
#         self.embedding_item = torch.nn.Embedding(sizes_ti[0],sizes_ti[1],
#                                                             _weight=model_dict['iid_embedding.weight'])
#         sizes_tu = model_dict['uid_embedding.weight'].size()
...
class MFBasedModel(torch.nn.Module):
    ...
    def forward(self, x, samples=None, stage=None):
        if stage == 'train_src':
            ...
            # iid_emb = self.src_pretrain_model.item_embedding_list[1](x[:, 1]).unsqueeze(1).to(self.device)  # [128, 10])
            iid_emb = self.src_pretrain_model.embedding_item(x[:, 1]).unsqueeze(1).to(self.device)  # [128, 10])
            # uid_emb = self.src_pretrain_model.user_embedding_list[1](x[:, 0]).unsqueeze(1).to(self.device)     # [128, 10])
            uid_emb = self.src_pretrain_model.embedding_user(x[:, 0]).unsqueeze(1).to(self.device)     # [128, 10])
            src1_user_emb = self.src_pretrain_model.user_embedding_list[0](x[:, 0].unsqueeze(1)).to(self.device)  # [128, 1, 10])
...
#             ufea = self.tgt_model.embedding_item(x[:, 2:]).to(self.device)   # [128, 20, 10])
            ...
            # ufea4 = self.src_pretrain_model.item_embedding_list[1](x[:, 2:]).to(self.device)
            ufea4 = self.src_pretrain_model.embedding_item(x[:, 2:]).to(self.device)

            # mapping1 = self.meta_net.forwardV8_1(src1_user_emb, ufea1, x[:, 2:]).to(self.device)
            # mapping2 = self.meta_net.forwardV8_1(src2_user_emb, ufea2, x[:, 2:]).to(self.device)
            # mapping3 = self.meta_net.forwardV8_1(src3_user_emb, ufea3, x[:, 2:]).to(self.device)
            # mapping4 = self.meta_net.forwardV8_1(uid_emb, ufea4, x[:, 2:]).to(self.device)
            ...
            src1_emb = torch.bmm(src1_user_emb, mapping1).to(self.device)  # [128, 10])
            ...
            # attn_output =self.target_attention.forward_avg(src_map_seq_emb).to(self.device)
            ...
            # iid_emb = self.src_pretrain_model.item_embedding_list[1](x[:, 1].unsqueeze(1)).to(self.device)  # [128, 1, 10])
            iid_emb = self.src_pretrain_model.embedding_item(x[:, 1]).unsqueeze(1).to(self.device)
            # uid_emb = self.src_pretrain_model.user_embedding_list[1](x[:, 0].unsqueeze(1)).to(self.device)
...
#             ufea = self.tgt_model.embedding_item(x[:, 2:]).to(self.device)   # [128, 20, 10])
            ...
            src1_emb = torch.bmm(src1_user_emb, mapping1).to(self.device)  # [128, 1, 10])
            src2_emb = torch.bmm(src2_user_emb, mapping2).to(self.device)
            src3_emb = torch.bmm(src3_user_emb, mapping3).to(self.device)
            src4_emb = torch.bmm(uid_emb, mapping4).to(self.device)

            src_map_seq_emb = torch.cat([src1_emb, src2_emb, src3_emb,src4_emb], 1).to(self.device)   # [128, 3, 10])
            attn_output =self.target_attention.forward(iid_emb,src_map_seq_emb).to(self.device)    # [128, 10] # [128, 10]
            # attn_output =self.target_attention.forward_avg(src_map_seq_emb).to(self.device)            
